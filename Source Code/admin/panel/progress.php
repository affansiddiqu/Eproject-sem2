...
$fetch = "SELECT * FROM `form` where status = 'In Progress' order by fid desc limit {$offset}, {$limit}";
            ...
            <table class="table table-warning">
                ...
                <tbody>
                  <?php while ($row = mysqli_fetch_assoc($data)) {
                    ...
                    <td><a href="del.php?fid=<?php echo $row['fid'];?>"><button type="submit" class="btn btn-success " name="submit">Delivered</button></a></td>
                  </tr>
                <?php
                    }
                ?>
...
$query = mysqli_query($connection, $fetchpage);
  ...
  if(mysqli_num_rows($query) > 0){
    ...
    if($getpgno > 1){
      echo '<li class="page-item"><a class="page-link" href="progress.php?page='.($getpgno - 1).'">prev</a></li>';

    }
    for($i = 1; $i <= $totalpages; $i++){
      $active = $i == $getpgno? "active" : "";
      echo '<li class="'.$active.' page-item"><a class="page-link" href="progress.php?page='.$i.'">'.$i.'</a></li>';
    }
    if($getpgno < $totalpages){
      echo '<li class="page-item"><a class="page-link" href="progress.php?page='.($getpgno + 1).'">next</a></li>';
    }
